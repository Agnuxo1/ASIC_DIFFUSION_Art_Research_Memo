...
def get_silicon_signature(image_path, api_host="127.0.0.1", api_port=4000):
    ...
    try:
        ...
        # 5. Embed Robust RS Watermark (Anti-Tamper)
        try:
            print("[SIGN] Applying Robust LSB Watermark...")
            from silicon_rs_watermark import embed_watermark
            ...
            # Helper to auto-path the RS version
            embed_watermark(auth_path, sig_dict, auth_path) # Overwrite auth_path with watermarked version
            print(f"[DONE] Robust RS Watermark embedded into {auth_path}")
            
        except ImportError:
            print("[WARN] silicon_rs_watermark module not found. Skipping robust watermark.")
        except Exception as e:
            print(f"[WARN] Failed to embed robust watermark: {e}")
